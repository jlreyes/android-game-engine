...
public class SineFunction implements Function1<FloatWrapper, Void> {
    private float mMagnitude;
    private float mHalfPeriod;
    private float mA;
    private FloatWrapper mFloatWrapper;

    public SineFunction(float magnitude, float halfPeriod) {
    ...
    public Void run(FloatWrapper d) {
        float x = d.getFloat();
        float y = mMagnitude * FloatMath.sin(mA * x);
        d.setFloat(y);
        return null;
    }
